...
namespace Hybrasyl
{
    public class Client
    {
        ...
        private ConcurrentQueue<ClientPacket> RecvQueue = new ConcurrentQueue<ClientPacket>();
        ...
        public void ClientLoop()
        {
            while (Connected)
            {
                lock (SendQueue)
                {
                    ...
                    {
                        ServerPacket sendPacket;
                        if (SendQueue.TryDequeue(out sendPacket))
                        {
                            if (sendPacket.ShouldEncrypt)
                            {
                                ++serverOrdinal;
                                sendPacket.Ordinal = serverOrdinal;

                                sendPacket.GenerateFooter();
                                sendPacket.Encrypt(this);
                            }
                            if (sendPacket.TransmitDelay != 0)
                            {
                                Thread.Sleep(sendPacket.TransmitDelay);
                            }
                            var sendBytes = sendPacket.ToArray();
                            Socket.BeginSend(sendBytes, 0, sendBytes.Length, SocketFlags.None, new AsyncCallback(EndSend), Socket);
                        }
                ...
                lock (RecvQueue)
                {
                    ...
                    {
                        ClientPacket recvPacket;
                        if (RecvQueue.TryDequeue(out recvPacket))
                        {
                            if (recvPacket.ShouldEncrypt)
                            {
                                recvPacket.Decrypt(this);
                            }

                            if (recvPacket.Opcode == 0x39 || recvPacket.Opcode == 0x3A)
                                recvPacket.DecryptDialog();

                            if (Connected)
                            {
                                if (Constants.PACKET_THROTTLES.ContainsKey(recvPacket.Opcode))
                                {
                                    ThrottleInfo tinfo;

                                    if (Throttle.TryGetValue(recvPacket.Opcode, out tinfo))
                                    {
                                        tinfo.Received();
                                        if (tinfo.IsThrottled)
                                        {
                                            Logger.InfoFormat("cid {0}: opcode 0x{1:X2} throttled", ConnectionId, recvPacket.Opcode);
                                            if (tinfo.TotalThrottled > tinfo.Throttle.DisconnectAfter)
                                            {
                                                Logger.InfoFormat("cid {0}: opcode 0x{1:X2} throttle limit reached: disconnected", ConnectionId, recvPacket.Opcode);
                                                Connected = false;
                                                break;
                                            }
                                            recving = false;
                                    ...
                                    else
                                    {
                                        ...
                                        Logger.DebugFormat("cid {0}: creating throttle for opcode 0x{1:X2}", ConnectionId,
                                            recvPacket.Opcode);
                                        Throttle[recvPacket.Opcode] = new ThrottleInfo(recvPacket.Opcode);
                                    }
                                }
                                ...
                                try
                                {
                                    if (Server is Lobby)
                                    {
                                        Logger.DebugFormat("Lobby: 0x{0:X2}", recvPacket.Opcode);
                                        var handler = (Server as Lobby).PacketHandlers[recvPacket.Opcode];
                                        handler.Invoke(this, recvPacket);
                                        UpdateLastReceived();
                                    }
                                    else if (Server is Login)
                                    {
                                        var handler = (Server as Login).PacketHandlers[recvPacket.Opcode];
                                        handler.Invoke(this, recvPacket);
                                        Logger.DebugFormat("Login: 0x{0:X2}", recvPacket.Opcode);
                                        UpdateLastReceived();
                                    }
                                    else
                                    {
                                        if (recvPacket.Opcode != 0x45 && recvPacket.Opcode != 0x75)
                                        {
                                            // Heartbeats don't effect our idle calcuations
                                            UpdateLastReceived(recvPacket.Opcode != 0x45 && recvPacket.Opcode != 0x75);
                                        }
                                        Logger.DebugFormat("Queuing: 0x{0:X2}", recvPacket.Opcode);
                                        World.MessageQueue.Add(new HybrasylClientMessage(recvPacket, ConnectionId));

                                    }
                                }
                                catch (Exception e)
                                {
                                    Logger.ErrorFormat("EXCEPTION IN HANDLING: 0x{0:X2}: {1}", recvPacket.Opcode, e);
                                }
        ...
        private void EndReceive(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                while (fullRecvBuffer.Count > 3)
                {
                    ...
                    lock (RecvQueue)
                    {
                        RecvQueue.Enqueue(clientPacket);
                    }

                }
                //Socket.BeginReceive(recvBuffer, 0, recvBuffer.Length, SocketFlags.None, new AsyncCallback(EndReceive), null);
            }
            //    while (fullRecvBuffer.Count > 3)
            //    {
            //        Logger.Debug("in fullRecvBuffer loop");
            //        int length = ((int)fullRecvBuffer[1] << 8) + (int)fullRecvBuffer[2] + 3;

                //        if (length > fullRecvBuffer.Count)
                //            break;
                ...
                //        var packet = new ServerPacket(range);
