...
namespace Hybrasyl.Subsystems.Players.Grouping;
...
public class UserGroup
{
    ...
    private readonly DistributionFunc ExperienceDistributionFunc;
    private List<User> _expShareInRange = new();
    ...
    public UserGroup(User founder)
    {
        ...
        foreach (var cl in Enum.GetValues(typeof(Class)).Cast<Class>()) ClassCount[cl] = 0;
    ...
    public bool Add(User user)
    {
        ...
        if (user.Grouped)
        {
            ...
            if (Count == 1) Remove(Members[0]);

            return false;
        }

        // Send a message to notify everyone else that someone's joined.
        foreach (var member in Members) member.SendMessage(user.Name + " has joined your group.", MessageTypes.SYSTEM);
    ...
    {
        var share = ExperienceDistributionFunc(source, exp);
    ...
    {
        var share = new Dictionary<uint, uint>();

        foreach (var member in MembersWithinRange(source)) share[member.Id] = full;
    ...
    private Dictionary<uint, uint> Distribution_AllClassBonus(User source, uint full)
    {
        ...
        {
            var inRange =
                MembersWithinRange(source).Count -
