...
public class HybrasylDialogSequence
{
    ...
    internal DialogSequence Sequence { get; }
    ...
    /// <summary>
    ///     Add a menu check to the given dialog sequence. If this is a pursuit, the (boolean) expression will be evaluated
    ///     before the
    ...
    public void AddMenuCheckExpression(string check)
    {
        if (string.IsNullOrEmpty(check))
        {
            GameLog.ScriptingError(
                "AddMenuCheckExpression: lua expression (first argument) is null or empty, ignoring");
            return;
        }
    ...
    public void SetDisplayName(string displayName)
    {
        ...
        {
            GameLog.ScriptingError(
                "SetDisplayName: display name (first argument) is null or empty, setting to 'I-AM-ERROR'");
